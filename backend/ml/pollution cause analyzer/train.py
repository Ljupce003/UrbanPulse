...
Run:
    ...
    python train.py --data /path/to/imputed_dataset.csv
"""
...
def evaluate(model: xgb.Booster, X, y, features, label=""):
    ...
    logger.info(f"[{label}] MAE={mae:.3f}  RMSE={rmse:.3f}  R²={r2:.4f}")
...
def train(data_path: Path = DATA_PATH):
    logger.info("=" * 70)
    logger.info("  Pollution Cause Analyzer — Training Pipeline")
    logger.info("=" * 70)
    ...
    logger.info(f"Features ({len(features)}): {', '.join(features[:10])}{'...' if len(features)>10 else ''}")
    ...
    logger.info(f"Split → train:{len(X_tr):,}  val:{len(X_val):,}  test:{len(X_te):,}")
    ...
    logger.info(f"xgb_aqi.ubj           ({model_path.stat().st_size // 1024} KB)")
    logger.info(f"shap_explainer.pkl    ({explainer_path.stat().st_size // 1024} KB)")
    logger.info(f"feature_meta.json")
    logger.info(f"training_report.json")

    logger.info("=" * 70)
    logger.info(f"Training Completed Successfully!")
    logger.info(f"Test R² = {test_m['r2']:.4f} | MAE = {test_m['mae']:.2f} AQI points")
    logger.info("=" * 70)
