...
@router.get("/predict-median", response_model=TrafficPredictionResponse)
async def predict_with_median_traffic(
    city: str = "Prague",
    country_code: str = "CZ",
    lat: float | None = None,
    lon: float | None = None,
    ...
    input_data.update({
        ...
        "precip_mm": getattr(weather_response.weather, 'rain', 0) if hasattr(weather_response.weather, 'rain') else 0,
    ...
    final_input = {col: input_data[col] for col in feature_names if col in input_data}

    predicted_aqi = predict(final_input)
    ...
    return TrafficPredictionResponse(
        ...
        message="Prediction using latest historical row as base + current weather + median traffic"
    )